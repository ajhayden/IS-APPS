...
// This view contains the web view, which displays the actual html content
        ...
        VStack {
            WebView(html: viewModel.html, request: nil)
                .injectNavigationHandler { geoPlaceId in
                    // Set the proper latitude, longitude, latitudeDelta, and longitudeDelta given a set of geoLocations
                ...
                .sheet(isPresented: $displayModalDetailView) {
                    NavigationView {
                        MapView(viewModel: viewModel)
                            .navigationBarTitle(viewModel.currentLocation == "0" // Turnary that will remove 0 so "Introduction 0" is not displayed
